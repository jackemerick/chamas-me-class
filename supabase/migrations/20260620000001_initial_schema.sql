...
-- Verifica se o usuario autenticado e superadmin
...
-- Retorna os org_ids aos quais o usuario pertence
...
-- Retorna o role do usuario em uma org especifica
...
create table public.classes (
  ...
  org_id      uuid not null references public.organizations(id) on delete cascade,
  name        text not null,
  group_label text,                         -- ex: "Juniores", "Adolescentes"
  created_by  uuid references auth.users(id) on delete set null,
...
-- ============================================================
-- CLASS_TEACHERS (professores vinculados a turma)
-- ============================================================
...
-- ============================================================
-- COURSES (cursos dentro de uma turma)
-- ============================================================
...
-- ============================================================
-- MEETING_TASKS (tarefas de um encontro)
-- ============================================================
...
-- ============================================================
-- POINT_CATEGORIES (categorias de ponto por turma)
-- ============================================================
...
-- ============================================================
-- CERT_CRITERIA (criterios de certificado por turma/curso)
-- ============================================================
...
create table public.cert_criteria (
  ...
  min_attendance_pct  int not null default 75,   -- percentual minimo de presenca (0-100)
...
create table public.certificates (
  ...
  pdf_url    text,                                 -- storage URL apos geracao
  created_at timestamptz not null default now()
);
...
create policy "superadmin ve tudo" on public.organizations
...
create policy "admin atualiza propria org" on public.organizations
  for update using (
    public.my_role_in_org(id) = 'admin'
  );
...
create policy "superadmin ve tudo" on public.org_members
  for all using (public.is_superadmin());
...
create policy "admin gerencia membros" on public.org_members
  for all using (
    public.my_role_in_org(org_id) = 'admin'
  );
...
create policy "superadmin ve tudo" on public.classes
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.class_teachers
  for all using (public.is_superadmin());

create policy "member da org ve professores" on public.class_teachers
  for select using (
    class_id in (
      select id from public.classes where org_id in (select public.my_org_ids())
    )
  );

create policy "admin gerencia professores" on public.class_teachers
  for all using (
    class_id in (
...
create policy "superadmin ve tudo" on public.courses
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.students
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.meetings
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.meeting_tasks
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.attendance
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.point_categories
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.points
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.cert_criteria
  for all using (public.is_superadmin());
...
create policy "superadmin ve tudo" on public.certificates
  for all using (public.is_superadmin());
...
-- ============================================================
-- Rodar manualmente apos criar o usuario no Supabase Auth:
--
